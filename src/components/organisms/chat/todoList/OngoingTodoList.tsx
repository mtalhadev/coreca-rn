...
type NavProps = StackNavigationProp<RootStackParamList, 'AdminOngoingTodoList'>
type RouteProps = RouteProp<RootStackParamList, 'AdminOngoingTodoList'>
...
const OngoingTodoList = (props: Partial<SwitchAdminOrWorkerProps>) => {
    const { t } = useTextTranslation()
    const navigation = useNavigation<NavProps>()
    const route = useRoute<RouteProps>()
    const side = props.side ?? 'admin'
    const screenName: string = side == 'admin' ? 'AdminOngoingTodoList' : 'WorkerOngoingTodoList'
    ...
    useEffect(() => {
        ;(async () => {
            try {
                ...
                const todosResult: CustomResponse<GetTodoListResponse> = await getTodoList({ status: 'ongoing' })
    ...
    const _onPressTodo = async (todoId?: string) => {
        const updateTodoList = [ ...filteredTodos ]
        const index = filteredTodos.findIndex(item => item.todoId === todoId)
        if(index > -1){
            ...
            console.log('updateTodoList: ', updateResponse.success?.items);
