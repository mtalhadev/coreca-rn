...
import { getNotifications, getUnreadNotificationCountOfTargetAccount, updateAlreadyReadNotifications } from '../../usecases/notification/CommonNotificationCase'
...
const initialPage = { all: 0, transaction: 0, site: 0, others: 0 }
const initialIsLastItemOfTheList = { all: false, transaction: false, site: false, others: false }
const initialIsContentsFetched = { all: true, transaction: true, site: true, others: true }
const initialLastItemCreatedAt = {
    all: nextDay(newCustomDate()).totalSeconds,
    transaction: nextDay(newCustomDate()).totalSeconds,
...
const AdminNotification = () => {
            ...
            try {
                ...
                if (getUnreadCountResult.error) {
                    throw {
                        ...
                        errorCode: getUnreadCountResult.errorCode,
                    }
                }
    ...
    useEffect(() => {
        if (accountNotificationList !== undefined && displayAccountNotificationList !== undefined) {
            ...
            const newDisplayAccountNotificationList = {
                ...cloneDeep(displayAccountNotificationList),
                [contentsType]: { items: [...cloneDeep(currentTotalItemList), ...cloneDeep(fetchedItemList)] },
            }
    ...
    useEffect(() => {
        if (isAllUnread && displayAccountNotificationList !== undefined) {
            for (let contentsType of ['all', 'transaction', 'site', 'others'] as NotificationsTagType[]) {
            ...
            if (accountId && !isAllUnReadRef.current) {
                try {
                    if (uniqueNotificationIds.current.length > 0) {
                        ;(async () => await updateAlreadyReadNotifications(uniqueNotificationIds.current, accountId, 'admin'))()
                    }
                } catch (error) {
                    const _error = error as CustomResponse
                    dispatch(
    ...
    useEffect(() => {
        if (!isAllUnReadRef.current && uniqueNotificationIds.current.length > 0) {
            try {
                ;(async () => {
                    const result = await updateAlreadyReadNotifications(uniqueNotificationIds.current, accountId, 'admin')
                    if (result.error) {
                        throw {
                            error: result.error,
    ...
    const onContentsChanged = (value: NotificationsTagType) => {
        setState((prev) => ({ ...prev, contentsType: value }))
    }
